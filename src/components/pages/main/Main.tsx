...
const MainPageContainer: React.FC<{ className?: string }> = ({ className }) => {
	const accounts: Account[] = useSelector(selectAccounts);
	const [incomes, setIncomes] = useState<Record[] | null>(null);
	const [expenses, setExpenses] = useState<Record[] | null>(null);
	const [lastRecords, setRecords] = useState<Record[] | null>(null);
		...
		<section className={className}>
			{isAuthenticated ? (
				<>
					<ErrorBoundary>
						<Windows>
							<Window title="Incomes" items={incomes} isLoading={isLoading} />
							<Window title="Accounts" items={accounts} isLoading={isLoading} />
							<Window title="Expenses" items={expenses} isLoading={isLoading} />
						</Windows>
					</ErrorBoundary>
